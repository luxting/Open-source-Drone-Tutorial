...
class QRCodeReader:
    def __init__(self, mapping_file, serial_port="/dev/ttyUSB0", baudrate=9600):
        ...
        self.serial_command_pending = False
    ...
    def image_callback(self, data):
        ...
        for obj in decoded_objects:
            new_qr_code_data = obj.data.decode("utf-8")
            if new_qr_code_data != self.current_qr_code_data:
                self.current_qr_code_data = new_qr_code_data
        ...
        # 设置pending标志
        self.serial_command_pending = True
        self.check_and_send_serial()

    def box_info_callback(self, data):
        new_box_info = data.data
        if new_box_info != self.current_box_info:
            self.current_box_info = new_box_info
    ...
    def check_and_send_serial(self):
        # 判断box_info和qr码信息是否都发生改变
        if self.serial_command_pending and (self.current_box_info != self.previous_box_info and self.current_qr_code_data != self.previous_qr_code_data):
            try:
                ...
                time.sleep(0.05)
                self.serial_command_pending = False  # 重置pending标志
                self.previous_box_info = self.current_box_info  # 更新previous_box_info
                self.previous_qr_code_data = self.current_qr_code_data  # 更新previous_qr_code_data
            except ValueError:
